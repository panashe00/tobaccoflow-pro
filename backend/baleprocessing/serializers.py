...
class BaleProcessingSerializer(serializers.ModelSerializer):
    class Meta:
        ...
        fields = ['id', 'bale', 'buyer_grade', 'price_per_kg', 'grade_mismatch', 'price_mismatch', 'processed_at']
        read_only_fields = fields

    def validate_buyer_grade(self, value):
        buyer = self.context.get('buyer')
        if buyer and not BuyerGrade.objects.filter(buyer=buyer, code__iexact=value, is_active=True).exists():
